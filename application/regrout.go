...
func (s *Service) applyRegrout(state *persistence.State, cmd Command, now domain.LogicalTime) (Result, error) {
	...
	state.Generations[persistence.GenKey(cmd.TaskID, nextGen)] = &domain.GenerationState{
		TaskID:           cmd.TaskID,
		Generation:       nextGen,
		ParentGeneration: task.CurrentGeneration,
		Reason:           cmd.Reason,
		Prefix:           []domain.PortID{},
		RecheckSet:       recheck,
		Defects:          append([]domain.SocketID(nil), cmd.Defects...),
	}
