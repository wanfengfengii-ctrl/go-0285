...
// Load replays and verifies the log on startup, and Save writes the log before
// publishing a new snapshot via fsync + atomic rename.
...
// appends any new evidence envelopes to the log, fsyncs, then atomically
...
func (r *FileRepository) Save(state *State, expectedVersion int64) error {
	...
	// Append any new evidence envelopes to the log before publishing.
